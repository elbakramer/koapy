...
기본 동작으로는 각 종목별 데이터를 ``${code}.xlsx`` 와 같은 파일명을 가지는 각 엑셀 파일로 ``datadir`` 하단에 생성해 저장합니다.
...
* 데이터를 가져온 뒤에 추가적인 후처리를 거의 하지 않아도 됩니다.
  키움의 경우는 가져온 데이터에 대해서 앞쪽에 붙어나오는 0 들을 제거한다거나
  전날대비기호가 붙어나오는 가격값들을 절대값으로 변환해 처리한다던가 하는 부수적인 작업이 필요합니다.
* CybosPlus 를 실행해 로그인을 최초 수동으로 해놓으면 이후 매번 스크립트를 수행할 때마다 따로 로그인할 필요가 없습니다.
    * 앞서 키움 OpenAPI 를 사용하는 경우에서 KOAPY 에서 제공하는 서버 같은 걸 별도로 띄우는 프로세스 같은게 딱히 필요가 없습니다.
