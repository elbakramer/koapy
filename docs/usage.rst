...
해당 메서드 목록은 `OpenAPI 개발 가이드`_ 에서 확인 가능합니다.
이후 그런 기본 메서드들을 활용하는 상위 함수들이 구현된 여러 래퍼 클래스들이 단계적으로 적용되면서
최종적으로 ``KiwoomOpenApiContext`` 에서 사용 가능한 모든 메서드들이 합쳐지는 구조입니다.
...
아래는 전체적으로 최상단의 ``KiwoomOpenApiContext`` 부터 최하단의 ``KiwoomOpenApiQAxWidget`` 까지 어떠한 흐름으로 이어져있는지 도식화한 것입니다.
...
.. _`OpenAPI 개발 가이드`: https://download.kiwoom.com/web/openapi/kiwoom_openapi_plus_devguide_ver_1.5.pdf

CLI (More)
----------

CLI 는 아무 옵션 없이 ``koapy`` 명령어만 실행했을 때 출력되는 설명문을 먼저 참고하세요.
